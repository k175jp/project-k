<script>
	...
	async function getProblemRandom() {
		const response = await fetch('http://192.168.7.38:8000/question/get', {
			method: 'GET',
			headers: {
    ...
    async function searchKeyword() {
		const response = await fetch(`http://192.168.7.38:8000/question/search?q=${keyword}`, {
			method: 'GET',
			headers: {
				'Authorization': "Bearer " + localStorage.getItem('token')
 		},
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
	...
	<Card href="/" class="m-8 w-2xl">
		...
		<div class="p-8">
		<Button color="blue" href="http://192.168.7.5:5000/answer-question/{question.id}">
    ...
    <Button color="green" href="http://192.168.7.5:5000/answer-question/{question.id}?mistakes=true">
